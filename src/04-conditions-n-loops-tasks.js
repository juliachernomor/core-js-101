...
// return(5*4) ==>  return (5*4)*3 ===> (5*4*3)*2
...
function findFirstSingleChar(str) {
  ...
  const massResult = Object.entries(obj).find((submass) => submass[1] === 1);
  return massResult === undefined ? null : massResult[0];
}
...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  const isStartIncludedBrackets = {
    false: '(',
    true: '[',
  };
  const start = isStartIncludedBrackets[isStartIncluded];
  const isEndIncludedBrackets = {
    false: ')',
    true: ']',
  };
  const end = isEndIncludedBrackets[isEndIncluded];
...
function isCreditCardNumber(ccn) {
  const sum1 = [...String(ccn)].reverse().filter((_, index) => index % 2 === 0)
    .reduce((acc, item) => acc + Number(item), 0);
  const sum2 = [...String(ccn)].reverse().filter((_, index) => index % 2 !== 0)
    .map((letter) => {
      if ((Number(letter) * 2) > 9) {
        return Number(letter) * 2 - 9;
      }
...
function getDigitalRoot(num) {
  const n = String(num).split('').reduce((acc, item) => acc + Number(item), 0);
...
function isBracketsBalanced(str) {
  ...
  try {
    str.split('').forEach((bracket) => {
      if (!isBrackets[bracket]) {
        stack.push(bracket);
      } else if (stack[stack.length - 1] === isBrackets[bracket]) {
        stack.pop(bracket);
      } else {
